...
async function main() {
  ...
  // Deploy CornerstoneToken implementation
  console.log("Deploying CornerstoneToken implementation...");
  const TokenImpl = await hre.ethers.getContractFactory("CornerstoneToken");
  const tokenImpl = await TokenImpl.deploy();
  await tokenImpl.waitForDeployment();
  const tokenImplAddr = await tokenImpl.getAddress();
  console.log("CornerstoneToken Implementation:", tokenImplAddr);

  // Deploy CornerstoneProject implementation
  console.log("Deploying CornerstoneProject implementation...");
  const ProjectImpl = await hre.ethers.getContractFactory("CornerstoneProject");
  const projectImpl = await ProjectImpl.deploy();
  await projectImpl.waitForDeployment();
  const projectImplAddr = await projectImpl.getAddress();
  console.log("CornerstoneProject Implementation:", projectImplAddr);

  // Deploy ProjectRegistry with implementations
  console.log("Deploying ProjectRegistry...");
  const Reg = await hre.ethers.getContractFactory("ProjectRegistry");
  const reg = await Reg.deploy(usdc, projectImplAddr, tokenImplAddr);
  await reg.waitForDeployment();
  const registry = await reg.getAddress();
  console.log("ProjectRegistry:", registry);

  if (
    String(process.env.CREATE_SAMPLE_PROJECT || "").toLowerCase() === "true"
  ) {
    console.log(
      "CREATE_SAMPLE_PROJECT=true → creating example Cornerstone project...",
    );
